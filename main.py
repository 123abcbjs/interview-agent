...
def relevant_lines(text: str, query: str, limit: int = 4) -> str:
    """Use simple keyword matching to select a few relevant source lines."""
    keywords = {
        word.lower()
...
def build_graph(model: ChatDeepSeek):
    def analyze_materials(state: InterviewState):
        prompt = f"""你是一名技术面试助手。请阅读岗位 JD 和候选人简历，
...
"""
    ...
    def generate_question(state: InterviewState):
        ...
        prompt = f"""你是一名友好但认真的技术面试官，现在进行第 {round_number} 轮面试。
只生成一个简洁、适合本科生 Agent 开发实习岗位的问题。
如果下一步是 FOLLOW_UP，请针对上一轮回答中不清楚的地方追问；
否则切换到尚未考察的重要方向。不要输出答案或解释。
...
"""
        ...
        focus = focus_match.group(1).strip() if focus_match else "下一个岗位核心能力"
    ...
    def generate_report(state: InterviewState):
        ...
        prompt = f"""请根据以下三轮技术面试生成一份简洁的Markdown面试报告。
必须包含：总体评价、回答证据、优势、待提升项、后续学习建议。
评价必须引用候选人的实际回答，不要虚构经历。
...
def parse_args():
    parser = argparse.ArgumentParser(description="基于 LangGraph 的自适应技术面试 Agent")
