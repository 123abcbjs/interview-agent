...
这是一个用于学习 LangGraph 的简单命令行项目。程序读取法律岗位 JD 和候选人简历，
进行 3 轮法律岗位模拟面试，并根据上一轮回答决定继续追问还是切换方向。面试结束后
会生成一份 Markdown 报告。

## 功能

- 使用 LangGraph 组织分析、提问、评价和报告生成流程
- 使用 DeepSeek 生成法律岗位问题并评价回答
- 重点考察法律基础、实务分析、证据与风险意识
...
- 模型可能产生不准确的法律表述，输出不构成法律意见，重要结论应由专业人员复核。
